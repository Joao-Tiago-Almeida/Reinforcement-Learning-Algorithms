...
for i in EPISODES:
    ...
    while not done:
        ...
        if SAVE_VIDEO: imgs.append(env.render("rgb_array"))
    ...
    if SAVE_VIDEO: clips.append(imgs)
...
if SAVE_VIDEO: 
# save best clip
    clip = clips[np.argmax(episode_reward_list)]
    video = cv2.VideoWriter(filename = 'car.mp4',
                            fourcc = cv2.VideoWriter_fourcc(*'mp4v'),
                            fps = 60.,
                            frameSize = (clip[0].shape[1],clip[0].shape[0]),
                            isColor = True )
